...
#define GPIODATA_TOGGLE(MASK,PORT_ID)  *((GPIO_RegAddType)(GPIO_REG_ADDRESS(PORT_ID,0x000) + (MASK << 2))) ^= 0xff
...
GPIO_CheckType GPIO_Toggle(uint8_t GroupId)
{
    ...
    if ((GroupId < GPIO_GROUPS_NUMBER))
    {
        ...
        if ((CfgWrite->PortDirection == 0xff) && (CfgWrite->SetPinType == 0xff)
                && (CfgWrite->UseAlterFun == 0x00)
                && (GPIO_GroupState[GroupId] == 1))
        {

            GPIODATA_TOGGLE(CfgWrite->PortMask, CfgWrite->PortId);
            RetVar = GPIO_OK;
